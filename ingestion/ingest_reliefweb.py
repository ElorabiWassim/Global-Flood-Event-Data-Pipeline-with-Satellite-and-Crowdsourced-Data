...
a polite ``appname`` parameter:
    ...
    https://api.reliefweb.int/v2/disasters?appname=<your-app>&filter[field]=type&filter[value]=Flood
...
from config.settings import HTTP_TIMEOUT, RELIEFWEB_APPNAME
...
def _fetch_disasters(limit: int = DEFAULT_LIMIT) -> list[dict[str, Any]]:
    ...
    The v2 endpoint enforces an *approved* ``appname``. If our default appname
    isn't registered the API returns HTTP 403 with a body like::

        {"status": 403, "error": {"type": "AccessDeniedHttpException",
         "message": "You are not using an approved appname. Kindly request..."}}

    We surface that message verbatim so operators know exactly what to do
    (register the appname at https://apidoc.reliefweb.int/parameters#appname
    ...
    if r.status_code == 403:
        # Promote the API's own helpful message into the exception so it
        # reaches the ingestion log and the Airflow task UI.
        try:
            err = (r.json() or {}).get("error", {}).get("message")
        except ValueError:
            err = None
        msg = (
            f"ReliefWeb v2 rejected appname={RELIEFWEB_APPNAME!r} (HTTP 403). "
            f"{err or ''} "
            "Set RELIEFWEB_APPNAME in .env to an approved value "
